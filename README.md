...
GEMINI_API_KEY=             # optional if using groq
```

Search works without any API key. Only the "Ask a Question" mode requires one.
...
│       └── test_queries.json     # Test queries (populate relevant_ids for eval)
├── frontend/
...
To run the retrieval ablation (requires populating `relevant_ids` in `test_queries.json`):

```bash
python -m backend.evaluation.evaluate
```

Outputs a table comparing P@5, nDCG@10, and MRR across BM25 / Dense / Hybrid / Hybrid+Reranker.
